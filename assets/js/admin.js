...
(0, _tlite["default"])(function (el) {
  return el.className.indexOf('ema4wp-tooltip') > -1;
});

require('./admin/list-fetcher.js');

require('./admin/fields/zozoema-api-key.js');
...
window.ema4wp.helpers = helpers;
window.ema4wp.settings = settings;
window.ema4wp.tabs = tabs;

},{"./admin/fields/zozoema-api-key.js":2,"./admin/helpers.js":3,"./admin/list-fetcher.js":4,"./admin/list-overview.js":5,"./admin/settings.js":6,"./admin/show-if.js":7,"./admin/tabs.js":8,"tlite":35}],2:[function(require,module,exports){
"use strict";

function validate(evt) {
  var node = document.createElement('p');
  node.className = 'help red';
  node.innerText = window.ema4wp_vars.i18n.invalid_api_key;
...
var field = document.getElementById('zozoema_api_key');
...
var config = window.ema4wp_vars;
var i18n = window.ema4wp_vars.i18n;
...
function fetch(evt) {
  ...
  m.request({
    method: 'POST',
    url: ajaxurl + '?action=ema4wp_renew_zozoema_lists',
...
function view() {
  return m('form', {
    ...
    type: 'submit',
    value: state.working ? i18n.fetching_zozoema_lists : i18n.renew_zozoema_lists,
    className: 'button',
    disabled: !!state.working
  }), m.trust(' &nbsp; '), state.working ? [m('span.ema4wp-loader', 'Loading...'), m.trust(' &nbsp; ')] : '', state.done ? [state.success ? m('em.help.green', i18n.fetching_zozoema_lists_done) : m('em.help.red', i18n.fetching_zozoema_lists_error)] : ''])]);
}

var mount = document.getElementById('ema4wp-list-fetcher');

if (mount) {
  // start fetching right away when no lists but api key given
  if (config.zozoema.api_connected && config.zozoema.lists.length === 0) {
    fetch();
  }
...
function showDetails(evt) {
  ...
  if (next.style.display === 'none') {
    m.request({
      method: 'GET',
      url: ajaxurl + '?action=ema4wp_get_list_details&ids=' + listID
...
var table = document.getElementById('ema4wp-zozoema-lists-overview');

if (table) {
  table.addEventListener('click', function (evt) {
    if (!evt.target.matches('.ema4wp-zozoema-list')) {
      return;
    }
...
function _open(tab, updateState) {
  ...
  if (window.ema4wp && window.ema4wp.forms && window.ema4wp.forms.editor) {
    window.ema4wp.forms.editor.refresh();
  }
