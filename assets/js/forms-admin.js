...
var i18n = window.ema4wp_forms_i18n;
...
var fieldHelperRootElement = document.getElementById('ema4wp-field-wizard');
...
var settings = window.ema4wp.settings;
var ajaxurl = window.ema4wp_vars.ajaxurl;
var i18n = window.ema4wp_forms_i18n;
var zozoema = window.ema4wp_vars.zozoema;
var countries = window.ema4wp_vars.countries;
...
/**
 * Normalizes the field type which is passed by ZozoEMA
 *
 * @param type
 * @returns {*}
 */
...
/**
 * Register a field for a ZozoEMA grouping
 *
...
function registerListsFields(lists) {
  var url = ajaxurl + '?action=ema4wp_get_list_details&ids=' + lists.map(function (l) {
    return l.id;
  }).join(',');
  m.request({
    url: url,
...
registerCustomFields(zozoema.lists);
...
function Field(data) {
  return {
    ...
    type: data.type,
    zozoemaType: data.zozoemaType || null,
...
var domDirty = false;
var editor;
var element = document.getElementById('ema4wp-form-content');
var previewFrame = document.getElementById('ema4wp-form-preview');
var previewDom;
var templateRegex = /\{[^{}]+\}/g;
/* functions */
...
function setPreviewDom() {
  ...
  previewDom = frameContent.querySelector('.ema4wp-form-fields');
...
function updatePreview() {
  ...
  previewDom.dispatchEvent(new Event('ema4wp-refresh'));
}
...
function findRequiredFields() {
  // query fields required by ZozoEMA
...
function render() {
  ...
  var container = document.querySelector('.ema4wp-notices');

  if (!container) {
    container = document.createElement('div');
    container.className = 'ema4wp-notices';
...
var groupingsNotice = function groupingsNotice() {
  var text = 'Your form contains deprecated <code>GROUPINGS</code> fields. <br /><br />Please remove these fields from your form and then re-add them through the available field buttons to make sure your data is getting through to ZozoEMA correctly.';
...
var requiredFieldsNotice = function requiredFieldsNotice() {
  ...
  var text = '<strong>Heads up!</strong> Your form is missing list fields that are required in ZozoEMA. Either add these fields to your form or mark them as optional in ZozoEMA.';
...
var zozoemaListsNotice = function zozoemaListsNotice() {
  var text = '<strong>Heads up!</strong> You have not yet selected a ZozoEMA list to subscribe people to. Please select at least one list from the <a href="javascript:void(0)" data-tab="settings" class="tab-link">settings tab</a>.';
...
document.body.addEventListener('change', zozoemaListsNotice);
...
function Overlay(vnode) {
  ...
  function view(vnode) {
    return [m('div.ema4wp-overlay-wrap', m('div.ema4wp-overlay', {
    ...
    m('span', {
      ...
      title: i18n.close,
      onclick: close
    }), vnode.children])), m('div.ema4wp-overlay-background', {
      title: i18n.close,
      onclick: close
    })];
  }
...
var context = document.getElementById('ema4wp-admin');
var listInputs = context.querySelectorAll('.ema4wp-list-input');
var lists = window.ema4wp_vars.zozoema.lists;
var selectedLists = [];
...
window.ema4wp.forms = window.ema4wp.forms || {};
window.ema4wp.forms.editor = editor;
