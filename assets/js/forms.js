...
function bind(evtName, cb) {
  document.addEventListener(evtName, function (evt) {
    ...
    if (typeof el.className === 'string') {
      fireEvent = el.className.indexOf('ema4wp-form') > -1;
    }

    if (!fireEvent && typeof el.matches === 'function') {
      fireEvent = el.matches('.ema4wp-form *');
    }
...
if (ema4wp.listeners) {
  var listeners = ema4wp.listeners;
  ...
  delete ema4wp.listeners;
} // expose forms object


ema4wp.forms = forms; // expose ema4wp object globally

window.ema4wp = ema4wp;
...
function evaluate() {
  var elements = document.querySelectorAll('.ema4wp-form [data-show-if],.ema4wp-form [data-hide-if]');
  [].forEach.call(elements, toggleElement);
} // re-evaluate conditional elements for change events on forms


function handleInputEvent(evt) {
  if (!evt.target || !evt.target.form || evt.target.form.className.indexOf('ema4wp-form') < 0) {
    return;
  }
...
document.addEventListener('ema4wp-refresh', evaluate, true);
...
Form.prototype.setResponse = function (msg) {
  this.element.querySelector('.ema4wp-response').innerHTML = msg;
}; // revert back to original state


Form.prototype.reset = function () {
  this.setResponse('');
  this.element.querySelector('.ema4wp-form-fields').style.display = '';
  this.element.reset();
};
...
function get(formId) {
  ...
  var formElement = document.querySelector('.ema4wp-form-' + formId);
