...
Donate link: https://www.ema4wp.com/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=donate-link
...
<p><a href="https://www.ema4wp.com/features/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=after-features-link">View more Premium features</a></p>
</blockquote>
...
If you are not yet using ZozoEMA, [creating an account is 100% free and only takes you about 30 seconds](http://zozoema.com/monkey-rewards/?utm_source=freemium_newsletter&utm_medium=email&utm_campaign=monkey_rewards&aid=a2d08947dcd3683512ce174c5&afl=1).
...
1. Set [your API key](https://admin.zozoema.com/account/api) in the plugin settings.
...
#### Need help?
Please take a look at the [ZozoEMA for WordPress knowledge base](https://www.ema4wp.com/kb/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=installation-instructions-link) first.
...
#### More documentation
More detailed documentation can be found in the [ZozoEMA for WordPress knowledge base](https://www.ema4wp.com/kb/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=faq).
...
[You can find your API key here](http://kb.zozoema.com/accounts/management/about-api-keys#Find-or-Generate-Your-API-Key)
...
Please head over to the [ZozoEMA for WordPress knowledge base](https://www.ema4wp.com/kb/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=faq) for more detailed documentation.
...
Please take a look at the [ZozoEMA for WordPress knowledge base](https://www.ema4wp.com/kb/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=description) as well.

#### Add-on plugins

There are several [add-on plugins](https://www.ema4wp.com/add-ons/#utm_source=wp-plugin-repo&utm_medium=zozoema-for-wp&utm_campaign=description) available, which help you get even more out of your site.

#### Translations

You can [help translate ZozoEMA for WordPress into your language](https://translate.wordpress.org/projects/wp-plugins/zozoema-for-wp/stable/) using your WordPress.org account.

#### Development

This plugin is being developed on GitHub. If you want to collaborate, please look at [ibericode/zozoema-for-wordpress](https://github.com/ibericode/zozoema-for-wordpress).

#### Customizing the plugin

The plugin provides various filter & action hooks that allow you to modify or extend default behavior. We're also maintaining a [collection of sample code snippets](https://github.com/ibericode/ema4wp-snippets).
...
- Add `EMA4WP_API_V3::add_template` method.
...
- Improved fetch and cache mechanism for retrieving ZozoEMA account details, fetching data only when it is required.
...
Please note that installing this update requires you to also update any add-ons like [ZozoEMA Top Bar](https://wordpress.org/plugins/zozoema-top-bar/) and [Zozo EMA for WordPress Premium](https://www.ema4wp.com/premium-features/) (if installed).
...
- Take birthday field format into account when sending data to ZozoEMA.
- Print Google reCAPTCHA script in footer.

**Changes**

- Changed plugin name to EMA4WP instead of ZozoEMA for WordPress.
...
- Show notice when form doesn't have a ZozoEMA list selected to subscribe people to.
...
- Built-in integration with Google reCAPTCHA to prevent bots from subscribing to your ZozoEMA lists.
...
- Renamed "ZozoEMA" to "ZozoEMA" to match ZozoEMA's new branding.
- More accurate handling of timeouts for accounts with many ZozoEMA lists.
- UX improvements for integrations overview page.
- Validate ZozoEMA API key format when it's entered.
...
- Added `EMA4WP_API_KEY` PHP constant which can be used to set your ZozoEMA API key.
- Add `ema4wp_zozoema_list_limit` filter hook to modify the maximum number of ZozoEMA lists to fetch. Defaults to 200.

**Improvements**

- Apply `ema4wp_integration_gravity-forms_options` filter hook on Gravity Forms integration options so the checkbox can be prechecked and the checkbox label text modified.
...
- Change Gravity Forms field name to "ZozoEMA for WordPress"
- Get rid of cached result of ZozoEMA API connection.
...
- Pre-check ZozoEMA list when creating a new form if there is only 1 list.
- Send `null` for unknown values in usage tracking data (only when opted-in).

**Additions**

- Add methods for accessing ZozoEMA's e-commerce promo code endpoints to API class.
...
- Add filter `ema4wp_http_request_args`.
...
- Add textual warnings to settings that may affect [GDPR compliance](https://www.ema4wp.com/kb/gdpr-compliance/).
...
- Improved [conditional fields logic](https://www.ema4wp.com/kb/conditional-fields-elements/).
- Debug log now includes request & response data.
- [Form JavaScript events](https://www.ema4wp.com/kb/javascript-form-events/) are fired in an isolated thread now, to prevent errors in event callbacks from breaking form functionality.
- Don't send empty field values to ZozoEMA when updating subscribers.
- Show interest grouping ID in list overview on settings page.

**Fixes**

- Ninja Forms export checkbox would always state "checked" when form contained a ZozoEMA sign-up checkbox.
...
- Dropdown fields with special characters were not properly passed to ZozoEMA.
- Interest groups with an all-numeric ID were not properly passed to ZozoEMA.
...
- Filter out empty array values when overriding selected ZozoEMA lists via `_ema4wp_lists`.
...
- Now logging all sign-ups from Ninja Forms integrations when using [Zozo EMA for WordPress Premium](https://www.ema4wp.com/premium-features/).

**Additions**

- Added Gravity Forms integration. You can now integrate with Gravity Forms by adding the "ZozoEMA" field to your forms.
...
- Show Akamai reference number when an API request is blocked by ZozoEMA's firewall.
...
- Fire action hook `ema4wp_form_updated_subscriber` whenever a form was used to update a subscriber in ZozoEMA.
- Increase browser timeout for AJAX request when fetching ZozoEMA lists.
...
- Randomise time of cron event that renews ZozoEMA lists.
- Always try to show ZozoEMA list info when API key is given.
...
- Improved ZozoEMA list fetching & memory usage for accounts with many lists.
...
- Added `ema4wp_integration_show_checkbox` filter.
...
- Ensure fields are HTML decoded before sending to ZozoEMA.
...
- Add `$args` parameter to `API::get_lists_activity` method. Relates to the [ZozoEMA Activity](https://wordpress.org/plugins/ema4wp-activity/) plugin.
...
- Show notice when form is missing required ZozoEMA fields.
...
- Add `data-hide-if` attribute logic to forms. See [conditionally hide form fields](https://www.ema4wp.com/kb/conditional-fields-elements/). Thanks to [Kurt Zenisek](http://kurtzenisek.com/).
...
- Added `ema4wp_form_api_error` action hook for API errors encountered by forms.
- Added `element_class` argument to `[ema4wp_form]` shortcode for adding CSS classes.
...
- Fetch ZozoEMA lists over AJAX, to speed up perceived performance (especially when your account has many lists).
- Periodically fetch ZozoEMA lists, so cache is always fresh.
...
- Add missing e-commerce related API methods for next week's [WooCommerce ZozoEMA e-commerce integration](https://www.ema4wp.com/kb/what-is-ecommerce/) release.
...
- Write field specific errors to debug log when ZozoEMA denies a sign-up request.
- Write to debug log when custom integrations can not find an EMAIL field.
- Differentiate between connection & authorization errors when testing connection to ZozoEMA.
- Bump limit of number of ZozoEMA lists to fetch from 100 to 500.
...
- Show connection errors on ZozoEMA settings page.

**Additions**

- Add pre-checked option to Ninja Forms integration.
- You can now [conditionally hide fields or elements](https://www.ema4wp.com/kb/conditional-fields-elements/) using the `data-show-if` attribute.
...
- Hidden fields which referenced interest groups by name were not sent to ZozoEMA.
...
This release updates the plugin to version 3 of the ZozoEMA API. Please [read through the upgrade guide](https://www.ema4wp.com/kb/upgrading-to-4-0/) before updating to make sure things keep working as expected for you.

**Changes**

- "Send welcome email" is now handled from your list settings in ZozoEMA.
- Filter `ema4wp_form_merge_vars` is now called `ema4wp_form_data`.
- Filter `ema4wp_integration_merge_vars` is now called `ema4wp_integration_data`.
- New format for GROUPING fields in forms & filter hooks.
- Value delimiter in hidden fields is now a pipe `|` character.

**Additions**

- New filter: `ema4wp_form_subscriber_data`.
- New filter: `ema4wp_integration_subscriber_data`.
- New form tag: `{cookie name="mycookie"}`

**Improvements**

- The plugin now communicates with the latest & greatest ZozoEMA API.
...
- Issue with only 10 ZozoEMA lists / fields / interests being returned.
...
- Add ZozoEMA API error code to debug log lines.
- Add plugin name + version to User-Agent header for all ZozoEMA API requests.
...
- API class fix for [eCommerce360 functionality](https://www.ema4wp.com/kb/what-is-ecommerce/).
...
- Some preparations for the upcoming migration to the new ZozoEMA API (version 3).
- Consistent hook parameters for `ema4wp_form_subscribed` action.
...
- The ZozoEMA API key is now obfuscated on the settings page.
...
- Issue with API array responses (for the [ZozoEMA Activity add-on](https://wordpress.org/plugins/ema4wp-activity/), for example).
...
- Deselecting all ZozoEMA lists wouldn't persist after saving form settings.
...
- Use all ZozoEMA lists for Lists Choice field, instead of just the selected ones.
...
- Make sure integrations have a ZozoEMA list selected before trying to subscribe.
- Move less important settings to "Other" page.
- When a field is required in ZozoEMA, it has to be required in forms as well now.
- Allow including a `_ema4wp_email_type` field in forms to set an explicit email type.
- Miscellaneous overall performance improvements.

**Additions**

- Added [debug logging](https://www.ema4wp.com/kb/how-to-enable-log-debugging/), which shows all warnings & errors the plugin encountered in communicating with ZozoEMA.
- Add `get_lists_for_email( $email )` method to API class.
- Add `EMA4WP_Queue` class for better background processing of expensive operations.
...
- Ninja Forms integration can now use `ema4wp-` prefixed admin labels.
...
- Make sure `ema4wp_show_form()` works without passing a form ID.
...
- Add `position` key to `ema4wp_admin_menu_items` filter to set a menu position.
...
Added `ema4wp_use_sslverify` filter to disable or explicitly enable SSL certificate verification.
...
- Now using group ID's for interest grouping fields, so changing the group in ZozoEMA does not require updating your form code.
- Never load enabled integrations which are not installed.
- Reintroduce support for automatically sending `OPTIN_IP`

**Additions**

- Add filter: `ema4wp_form_data`, filters form data before it is processed.
...
- Errors returned by ZozoEMA are now logged for Forms as well.
- Pre-select ZozoEMA list if there's just one list in the connected account.
...
- Add form-specific classes to preview form element. This allows the [Styles Builder](https://www.ema4wp.com/premium-features/) to work with the Form Preview.
...
Version 3.0 is a total revamp of the plugin. For a quick overview of the changes, please [read this post on our blog](https://www.ema4wp.com/blog/whats-new-in-zozoema-for-wordpress-the-big-three-o/).

Before upgrading, please go through the [upgrade guide](https://www.ema4wp.com/kb/upgrading-to-3-0/) as some things have changed.

**Breaking Changes**

- Captcha fields: `{captcha}` field is now handled by the [Captcha add-on plugin](https://wordpress.org/plugins/ema4wp-captcha/).
- New dynamic content tags syntax: `{data_NAME}` is now `{data key="NAME"}`
- Event binding: `jQuery(document).on('subscribe.ema4wp','.ema4wp-form', function(){ ... })` is now `ema4wp.forms.on('subscribed', function(form) { ... })`
...
- Better support for ZozoEMA `address` fields.
- Better support for choice fields (eg groupings, list choice & country fields).
- All fields marked as `required` are now validated server-side as well (instead of just ZozoEMA required fields).
...
- The [premium plugin](https://www.ema4wp.com/) is now an add-on of this plugin.
...
- Introduced various new filter & action hooks, please see the new [code reference for developers](http://developer.ema4wp.com/) for more information.
...
- Preparations for [the upcoming ZozoEMA for WordPress version 3.0 release](https://www.ema4wp.com/blog/breaking-backwards-compatibility-in-version-3-0/).
...
- Error in Russian translation, causing a broken link on the ZozoEMA settings page.

**Improvements**

- Textual improvements to ZozoEMA settings page.
- Connectivity issues with ZozoEMA will now _always_ show an error message.
- Renewing ZozoEMA lists will now also update the output of the `{subscriber_count}` tag.
...
- Fix `ema4wp_form_before_fields` being applied twice.
- Position honeypot field to the right for Right-To-Left sites.
- `_ema4wp_lists` can now be a comma-separated string of ZozoEMA list ID's to subscribe to (or an array).
...
- ZozoEMA list fields of type "website" will now become HTML5 `url` type fields.
...
- Empty response from ZozoEMA API

**Improvements**

- Do not query ZozoEMA API for interest groupings if list has none.
...
- Showing "not connected" when the plugin was actually connected to ZozoEMA.
- Issue with `address` fields when `addr1` was not given.
- Comment form checkbox not outputted for some older themes.

**Improvements**

- Do not flush ZozoEMA cache on every settings save.
...
- ZozoEMA lists cache is now automatically flushed after changing your API key setting.
...
- Better error message when connected to ZozoEMA but account has no lists.

**Additions**

- Add `ema4wp_form_action` filter to set a custom `action` attribute on the form element.
...
- Filter `ema4wp_form_animate_scroll` now disables just the scroll animation, not the scroll itself.
- Revamped UI for ZozoEMA lists overview
- Updated German & Greek translations.

**Additions**

- Added `ema4wp_form_is_submitted()` and `ema4wp_form_get_response_html()` functions.

#### 2.3.7 - July 13, 2015

**Improvements**

- Use the same order as ZozoEMA.com, which is useful when you have over 100 ZozoEMA lists.
- Use `/* ... */` for inline JavaScript comments to prevent errors with minified HTML - props [Ed Gifford](https://github.com/egifford)

**Additions**

- Filter: `ema4wp_form_animate_scroll` to disable animated scroll-to after submitting a form.
...
- Relayed the browser's Accept-Language header to ZozoEMA for auto-detecting a subscriber's language.
...
- Updated language files.
- Now passing the form as a parameter to `ema4wp_form_css_classes` filter.

#### 2.3.4 - May 29, 2015

**Fixes**

- Issue with GROUPINGS not being sent to ZozoEMA
...
- Groupings not being sent to ZozoEMA
...
- You can now use [form variables in both forms, messages as checkbox label texts](https://www.ema4wp.com/kb/using-variables-in-your-form-or-messages/).
...
- Menu item for settings page not appearing on Google App Engine ([#88](https://github.com/ibericode/zozoema-for-wordpress/issues/88))
...
- Added `ema4wp_before_checkbox` and `ema4wp_after_checkbox` filters to easily add more fields to sign-up checkbox integrations.
- Added some helper methods related to interest groupings to `EMA4WP_ZozoEMA` class.
- Allow setting custom ZozoEMA lists to subscribe to using `lists` attribute on shortcode.
...
- Added [ema4wp_form_success](https://github.com/ibericode/zozoema-for-wordpress/blob/06f0c833027f347a288d2cb9805e0614767409b6/includes/class-form-request.php#L292-L301) action hook to hook into successful sign-ups
- Added [ema4wp_form_data](https://github.com/ibericode/zozoema-for-wordpress/blob/06f0c833027f347a288d2cb9805e0614767409b6/includes/class-form-request.php#L138-L142) filter hook to modify all form data before processing
...
- `ema4wp-submitted` class was not added in IE8
...
- Now showing a heads up when at limit of 100 ZozoEMA lists. ([#71](https://github.com/ibericode/zozoema-for-wordpress/issues/71))
- Added `wpml-config.xml` file for better WPML compatibility
- Added filter `ema4wp_menu_items` for adding & removing menu items from add-ons

#### 2.2.3 - January 24, 2015

Minor improvements and additions for compatibility with the [ZozoEMA User Sync plugin](https://www.ema4wp.com/premium-features/).

#### 2.2.2 - January 13, 2015

**Fixes**

- Plugin wasn't connecting to ZozoEMA for users on ZozoEMA server `us10` (API keys ending in `-us10`)
...
- "Select at least one list" notice appearing when unselecting any ZozoEMA list in Form settings
