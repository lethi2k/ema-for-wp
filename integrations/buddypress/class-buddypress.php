...
/**
 * Class EMA4WP_BuddyPress_Integration
 *
 * @ignore
 */
class EMA4WP_BuddyPress_Integration extends EMA4WP_User_Integration {
	...
	public function add_hooks() {
		...
		if ( is_multisite() ) {
			 ...
			 * activation process.
			 *
			 * To avoid all signups being subscribed to the ZozoEMA list until they
		 ...
		 * plugins to delay the ZozoEMA signup process.
		 *
		 * Plugins can hook into the 'ema4wp_integration_buddypress_should_subscribe' filter to prevent
		 * subscriptions from taking place:
		 *
		 * add_filter( 'ema4wp_integration_buddypress_should_subscribe', '__return_false' );
		 *
		 * The plugin would then then call:
		 *
		 * do_action( 'ema4wp_integration_buddypress_subscribe_user', $user_id );
		 *
		 * to perform the subscription at a later point.
		 */
		add_action( 'ema4wp_integration_buddypress_subscribe_user', array( $this, 'subscribe_buddypress_user' ), 10, 1 );
	}
	...
	public function subscribe_from_form( $user_id, $user_login, $user_password, $user_email ) {
		...
		/**
		 * Allow other plugins to prevent the ZozoEMA sign-up.
		 *
		 * @param bool $subscribe False does not subscribe the user.
		 * @param int $user_id The user ID to subscribe
		 */
		$subscribe = apply_filters( 'ema4wp_integration_buddypress_should_subscribe', $subscribe, $user_id );
	...
	public function subscribe_from_usermeta( $user_id, $key, $userdata ) {
		...
		if ( empty( $meta['ema4wp_subscribe'] ) ) {
			return false;
		}
		...
		/**
		 * @ignore Documented elsewhere, see EMA4WP_BuddyPress_Integration::subscribe_from_form.
		 */
		$subscribe = apply_filters( 'ema4wp_integration_buddypress_should_subscribe', $subscribe, $user_id );
		if ( ! $subscribe ) {
			return false;
	...
	/**
	 * Subscribes a user to ZozoEMA list(s).
	 *
	 * @param int $user_id The user ID to subscribe
	 * @return bool
	 */
