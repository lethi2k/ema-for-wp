...
/**
 * Class EMA4WP_WooCommerce_Integration
 *
 * @ignore
 */
class EMA4WP_WooCommerce_Integration extends EMA4WP_Integration {
	...
	*/
	public function save_woocommerce_checkout_checkbox_value( $order_id ) {
		update_post_meta( $order_id, '_ema4wp_optin', $this->checkbox_was_checked() );
	}
	...
	public function triggered( $order_id = null ) {
		...
		$do_optin = get_post_meta( $order_id, '_ema4wp_optin', true );
		return $do_optin;
	}
	...
	public function subscribe_from_klarna_checkout( $order_id, $klarna_order ) {
		...
		update_post_meta( $order_id, '_ema4wp_optin', true );
	...
	*/
	public function subscribe_from_woocommerce_checkout( $order_id ) {
		...
		// TODO: add billing address fields, maybe by finding ZozoEMA field of type "address"?
	...
	public function get_object_link( $object_id ) {
		return sprintf( '<a href="%s">%s</a>', get_edit_post_link( $object_id ), sprintf( __( 'Order #%d', 'zozoema-for-wp' ), $object_id ) );
	}
}
