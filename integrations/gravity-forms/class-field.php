...
class EMA4WP_Gravity_Forms_Field extends GF_Field {
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'ZozoEMA for WordPress', 'zozoema-for-wp' );
	}
	...
	public function get_form_editor_field_settings() {
		return array(
			...
			'description_setting',
			'css_class_setting',
			'zozoema_list_setting',
			'zozoema_double_optin',
			'zozoema_precheck',
			'rules_setting',
		);
	}
	...
	private function apply_ema4wp_options_filters( $options ) {
		$options = apply_filters( 'ema4wp_gravity-forms_integration_options', $options );
		$options = apply_filters( 'ema4wp_integration_gravity-forms_options', $options );
		return $options;
	}
	...
	public function get_checkbox_choices( $value, $disabled_text, $form_id = 0 ) {
		...
		$options = array(
			...
			'precheck' => isset( $this->zozoema_precheck ) ? $this->zozoema_precheck : false,
		);
		$options = $this->apply_ema4wp_options_filters( $options );
