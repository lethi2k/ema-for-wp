...
/**
 * Class EMA4WP_Contact_Form_7_Integration
 *
 * @ignore
 */
class EMA4WP_Contact_Form_7_Integration extends EMA4WP_Integration {
	...
	*/
	public function init() {
		if ( function_exists( 'wpcf7_add_form_tag' ) ) {
			wpcf7_add_form_tag( 'ema4wp_checkbox', array( $this, 'shortcode' ) );
		} else {
			wpcf7_add_shortcode( 'ema4wp_checkbox', array( $this, 'shortcode' ) );
		}
	 ...
	 *
	 * - _ema4wp_subscribe_contact-form-7
	 * - ema4wp-subscribe
	 *
	 * @return bool
	 */
	public function checkbox_was_checked() {
		...
		return ( isset( $data[ $this->checkbox_name ] ) && (int) $data[ $this->checkbox_name ] === 1 )
			|| ( isset( $data['ema4wp-subscribe'] ) && (int) $data['ema4wp-subscribe'] === 1 );
	}

	/**
	* Alter Contact Form 7 data.
	*
	* Adds ema4wp_checkbox to post data so users can use `ema4wp_checkbox` in their email templates
	*
	* @param array $data
	* @return array
	*/
	public function alter_cf7_data( $data = array() ) {
		$data['ema4wp_checkbox'] = $this->checkbox_was_checked() ? __( 'Yes', 'zozoema-for-wp' ) : __( 'No', 'zozoema-for-wp' );
		return $data;
	}
	 ...
	 *
	 * @todo improve smart guessing based on selected ZozoEMA lists
	 *
	...
	public function process( $cf7_form ) {
		...
		$parser = new EMA4WP_Field_Guesser( $this->get_data() );
