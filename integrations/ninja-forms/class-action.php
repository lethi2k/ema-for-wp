...
/**
 * Class EMA4WP_Ninja_Forms_Action
 */
final class EMA4WP_Ninja_Forms_Action extends NF_Abstracts_ActionNewsletter {

	/**
	 * @var string
	 */
	protected $_name = 'ema4wp_subscribe';
	...
	public function __construct() {
		...
		$this->_nicename = __( 'ZozoEMA', 'zozoema-for-wp' );
		...
		$this->_settings['double_optin'] = array(
			'name'    => 'double_optin',
			'type'    => 'select',
			'label'   => __( 'Use double opt-in?', 'zozoema-for-wp' ),
			'width'   => 'full',
			'group'   => 'primary',
			'value'   => 1,
			'options' => array(
		...
		$this->_settings['update_existing'] = array(
			...
			'type'    => 'select',
			'label'   => __( 'Update existing subscribers?', 'zozoema-for-wp' ),
			'width'   => 'full',
			'group'   => 'primary',
			'value'   => 0,
			'options' => array(
		...
		//            'type' => 'select',
		//            'label' => __( 'Replace existing interest groups?', 'zozoema-for-wp'),
		//            'width' => 'full',
		//            'group' => 'primary',
		//            'value' => 0,
		//            'options' => array(
	...
	*/
	...
	public function process( $action_settings, $form_id, $data ) {
		...
		// find "ema4wp_optin" type field, bail if not checked.
		foreach ( $data['fields'] as $field_data ) {
			if ( $field_data['type'] === 'ema4wp_optin' && empty( $field_data['value'] ) ) {
				return;
			}
		}
		...
		do_action( 'ema4wp_integration_ninja_forms_subscribe', $email_address, $merge_fields, $list_id, $double_optin, $update_existing, $replace_interests, $form_id );
	}
