...
/**
 * Class EMA4WP_Ninja_Forms_v2_Integration
 *
 * @ignore
 */
class EMA4WP_Ninja_Forms_V2_Integration extends EMA4WP_Integration {
	...
	public function register_field() {
		...
		ninja_forms_register_field( 'ema4wp-subscribe', $args );
	}
	...
	public function process( $id, $value ) {
		...
		foreach ( $fields as $field_id => $field_value ) {
			...
			// try admin label for "ema4wp-" prefixed fields, otherwise use general label
			$label = $ninja_forms_processing->get_field_setting( $field_id, 'admin_label' );
			if ( empty( $label ) || stripos( $label, 'ema4wp-' ) !== 0 ) {
		...
		// guess zozoema variables
		$parser = new EMA4WP_Field_Guesser( $pretty );
