 ...
 */
class EMA4WP_WPForms_Field extends WPForms_Field {
	...
	public function init() {
		...
		$this->defaults = array(
			0 => array(
				'label'   => __( 'Sign-up to our newsletter?', 'zozoema-for-wp' ),
				'value'   => '1',
				'default' => '',
			),
		);
	...
	public function field_options( $field ) {
		...
		// ZozoEMA list
		$this->field_option_zozoema_list( $field );
	...
	private function field_option_zozoema_list( $field ) {
		$zozoema = new EMA4WP_ZozoEMA();

		// Field option label
		$tooltip      = __( 'Select the ZozoEMA list to subscribe to.', 'zozoema-for-wp' );
		$option_label = $this->field_element(
			'label',
			$field,
			array(
				'slug'    => 'zozoema-list',
				'value'   => __( 'ZozoEMA list', 'zozoema-for-wp' ),
				'tooltip' => $tooltip,
			),
			false
		);
	...
	private function field_option_choices( $field ) {
		$tooltip = __( 'Set your sign-up label text and whether it should be pre-checked.', 'zozoema-for-wp' );
		...
		$option_label = $this->field_element(
			...
			$field,
			array(
				'slug'    => 'zozoema-checkbox',
				'value'   => __( 'Sign-up checkbox', 'zozoema-for-wp' ),
				'tooltip' => $tooltip,
			),
			false
		);
	...
	public function format( $field_id, $field_submit, $form_data ) {
		...
		$data = array(
			'name'      => $name,
			'value'     => empty( $field_submit ) ? __( 'No', 'zozoema-for-wp' ) : __( 'Yes', 'zozoema-for-wp' ),
