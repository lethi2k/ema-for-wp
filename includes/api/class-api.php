...
class EMA4WP_API {
	...
	/**
	 * @var boolean Boolean indicating whether the user is connected with ZozoEMA
	 */
	protected $connected;
	...
	private function show_error( $message ) {
		...
		add_settings_error( 'ema4wp-api', 'ema4wp-api-error', $message, 'error' );
		return true;
	}
	...
	private function show_connection_error( $message ) {
		$message .= '<br /><br />' . sprintf( '<a href="%s">' . __( 'Read more about common connectivity issues.', 'zozoema-for-wp' ) . '</a>', 'https://www.ema4wp.com/kb/solving-connectivity-issues/#utm_source=wp-plugin&utm_medium=zozoema-for-wp&utm_campaign=settings-notice' );
	...
	public function is_connected() {
		...
		if ( is_object( $result ) ) {
			...
			if ( isset( $result->error ) ) {
				$this->show_error( 'ZozoEMA Error: ' . $result->error );
			}
		}
	...
	/**
	 * Unsubscribes the given email or luid from the given ZozoEMA list
	 *
	 * @param string       $list_id
	...
	/**
	 * @see https://apidocs.zozoema.com/api/2.0/ecomm/order-add.php
	 *
	...
	/**
	 * @see https://apidocs.zozoema.com/api/2.0/ecomm/order-del.php
	 *
	...
	public function call( $method, array $data = array() ) {
		...
		if ( empty( $this->api_key ) ) {
			$this->error_message = 'Missing ZozoEMA API key.';
			$this->error_code    = 001;
			return false;
		}
		...
		$request_args = array(
			...
			'sslverify' => apply_filters( 'ema4wp_use_sslverify', true ),
		);
	...
	/**
	 * Get the request headers to send to the ZozoEMA API
	 *
	 * @return array
	 */
	private function get_headers() {
		...
		$headers = array(
			...
			'User-Agent' => 'ema4wp/' . EMA4WP_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' ),
		);
	...
	private function parse_response( $response ) {
		if ( is_wp_error( $response ) ) {
			throw new Exception( 'Error connecting to ZozoEMA. ' . $response->get_error_message(), (int) $response->get_error_code() );
		}
		...
		if ( $code !== 200 ) {
			$message = sprintf( 'The ZozoEMA API server returned the following response: <em>%s %s</em>.', $code, $message );
			...
			if ( $code === 403 ) {
				...
				if ( ! empty( $matches[1] ) ) {
					$message .= '</strong><br /><br />' . sprintf( 'This usually means that your server is blacklisted by ZozoEMA\'s firewall. Please contact ZozoEMA support with the following reference number: %s </strong>', $matches[1] );
				}
			}
		}
