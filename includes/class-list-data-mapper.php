...
/**
* Class EMA4WP_Field_Map
*
* @access private
* @since 4.0
* @ignore
*/
class EMA4WP_List_Data_Mapper {
	...
	*/
	public function __construct( array $data, array $list_ids ) {
		...
		$this->formatter = new EMA4WP_Field_Formatter();
		$this->zozoema = new EMA4WP_ZozoEMA();
	}

	/**
	* @return EMA4WP_ZozoEMA_Subscriber[]
	*/
	public function map() {
		$map = array();
	...
	*/
	protected function map_list( $list_id ) {
		$subscriber                = new EMA4WP_ZozoEMA_Subscriber();
		$subscriber->email_address = $this->data['EMAIL'];

		// find merge fields
		$merge_fields = $this->zozoema->get_list_merge_fields( $list_id );
		...
		if ( ! empty( $this->data['INTERESTS'] ) ) {
			$interest_categories = $this->zozoema->get_list_interest_categories( $list_id );
		...
		// find language
		/* @see http://kb.zozoema.com/lists/managing-subscribers/view-and-edit-subscriber-languages?utm_source=mc-api&utm_medium=docs&utm_campaign=apidocs&_ga=1.211519638.2083589671.1469697070 */
	...
	*/
	private function format_merge_field_value( $merge_field, $value ) {
		...
		*
		* Use this to format a field value according to the field type (in ZozoEMA).
		*
		* @since 3.0
		* @param string $value The value
		* @param string $field_type The type of the field (in ZozoEMA)
		*/
		$value = apply_filters( 'ema4wp_format_field_value', $value, $field_type );
