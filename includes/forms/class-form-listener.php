...
class EMA4WP_Form_Listener {
	...
	/**
	 * @var EMA4WP_Form The submitted form instance
	 */
	...
	public function listen() {
		if ( empty( $_POST['_ema4wp_form_id'] ) ) {
			return false;
		}

		// get form instance
		try {
			$form_id = (int) $_POST['_ema4wp_form_id'];
		...
		$request_data = $_POST;
		$request_data = ema4wp_sanitize_deep( $request_data );
	 ...
	 */
	public function process_subscribe_form( EMA4WP_Form $form ) {
		$result     = false;
		$zozoema  = new EMA4WP_ZozoEMA();
		$email_type = $form->get_email_type();
		$data       = $form->get_data();
		$ip_address = ema4wp_get_request_ip_address();

		/** @var EMA4WP_ZozoEMA_Subscriber $subscriber */
		$subscriber = null;
		 ...
		 * @deprecated 4.0
		 */
		$data = (array) apply_filters( 'ema4wp_form_merge_vars', $data, $form );

		// create a map of all lists with list-specific data
		$mapper = new EMA4WP_List_Data_Mapper( $data, $form->get_lists() );
		...
		foreach ( $map as $list_id => $subscriber ) {
			...
			/**
			 * Filters subscriber data before it is sent to ZozoEMA. Fires for both form & integration requests.
			 *
			...
			/**
			 * Filters subscriber data before it is sent to ZozoEMA. Only fires for form requests.
			 *
			 * @param EMA4WP_ZozoEMA_Subscriber $subscriber
			 */
			$subscriber = apply_filters( 'ema4wp_form_subscriber_data', $subscriber );

			// send a subscribe request to ZozoEMA for each list
			$result = $zozoema->list_subscribe( $list_id, $subscriber->email_address, $subscriber->to_array(), $form->settings['update_existing'], $form->settings['replace_interests'] );
		}
		...
		if ( ! is_object( $result ) || empty( $result->id ) ) {
			$error_code    = $zozoema->get_error_code();
			$error_message = $zozoema->get_error_message();
			...
			if ( (int) $zozoema->get_error_code() === 214 ) {
				...
				$log->error( sprintf( 'Form %d > ZozoEMA API error: %s %s', $form->ID, $error_code, $error_message ) );
				 ...
				 */
				do_action( 'ema4wp_form_api_error', $form, $error_message );
			}
		...
		if ( $result->status === 'subscribed' && $result->was_already_on_list ) {
			 ...
			 * @since 3.0
			 *
			 * @param EMA4WP_Form $form Instance of the submitted form
			 * @param string $email
			 * @param array $data
			 */
			do_action( 'ema4wp_form_updated_subscriber', $form, $subscriber->email_address, $data );
		} else {
		 ...
		 * @param EMA4WP_ZozoEMA_Subscriber[] $subscriber
		 */
		do_action( 'ema4wp_form_subscribed', $form, $subscriber->email_address, $data, $map );
	}

	/**
	 * @param EMA4WP_Form $form
	 */
	public function process_unsubscribe_form( EMA4WP_Form $form ) {
		$zozoema = new EMA4WP_ZozoEMA();
		$log       = $this->get_log();
		$result    = null;
		$data      = $form->get_data();
		...
		foreach ( $form->get_lists() as $list_id ) {
			$result = $zozoema->list_unsubscribe( $list_id, $data['EMAIL'] );
		}

		if ( ! $result ) {
			$form->add_notice( $form->messages['error'], 'error' );
			$log->error( sprintf( 'Form %d > ZozoEMA API error: %s', $form->ID, $zozoema->get_error_message() ) );
		 ...
		 * @param string $email
		 */
		do_action( 'ema4wp_form_unsubscribed', $form, $data['EMAIL'] );
	}

	/**
	 * @param EMA4WP_Form $form
	 */
	public function respond( EMA4WP_Form $form ) {
		...
		if ( $success ) {
			 ...
			 * @param EMA4WP_Form $form Instance of the submitted form
			 */
			do_action( 'ema4wp_form_success', $form );
		} else {
			 ...
			 * @since 3.0
			 *
			 * @param EMA4WP_Form $form The submitted form instance.
			 */
			do_action( 'ema4wp_form_error', $form );
			...
			foreach ( $form->errors as $error ) {
				 ...
				 *
				 * - ema4wp_form_error_error                     General errors
				 * - ema4wp_form_error_spam
				 * - ema4wp_form_error_invalid_email             Invalid email address
				 * - ema4wp_form_error_already_subscribed        Email is already on selected list(s)
				 * - ema4wp_form_error_required_field_missing    One or more required fields are missing
				 * - ema4wp_form_error_no_lists_selected         No ZozoEMA lists were selected
				 *
				 * @since 3.0
				 *
				 * @param   EMA4WP_Form     $form        The form instance of the submitted form.
				 */
				do_action( 'ema4wp_form_error_' . $error, $form );
			}
		}
		 ...
		 * @param EMA4WP_Form $form Instance of the submitted form.
		 */
		do_action( 'ema4wp_form_respond', $form );
