...
class EMA4WP_Form_Element {
	 ...
	 *
	 * - lists: Customized number of ZozoEMA list ID's to subscribe to.
	 ...
	 * @param array $config
	 */
	public function __construct( EMA4WP_Form $form, $id, array $config = array() ) {
		$this->form   = $form;
		$this->ID     = $id;
		$this->config = $config;
	...
	protected function get_visible_fields() {
		 ...
		 * @param EMA4WP_Form $form
		 * @param EMA4WP_Form_Element $element
		 * @since 2.0
		 */
		$visible_fields = (string) apply_filters( 'ema4wp_form_content', $content, $form, $element );
	...
	protected function get_hidden_fields() {
		...
		// hidden fields
		$hidden_fields  = '<label style="display: none !important;">' . __( 'Leave this field empty if you\'re human:', 'zozoema-for-wp' ) . ' ' . '<input type="text" name="_ema4wp_honeypot" value="" tabindex="-1" autocomplete="off" /></label>';
		$hidden_fields .= '<input type="hidden" name="_ema4wp_timestamp" value="' . time() . '" />';
		$hidden_fields .= '<input type="hidden" name="_ema4wp_form_id" value="' . esc_attr( $this->form->ID ) . '" />';
		$hidden_fields .= '<input type="hidden" name="_ema4wp_form_element_id" value="' . esc_attr( $this->ID ) . '" />';
		...
		if ( ! empty( $this->config['lists'] ) ) {
			...
			$hidden_fields .= '<input type="hidden" name="_ema4wp_lists" value="' . esc_attr( $lists_string ) . '" />';
		}

		// was "lists" parameter passed in shortcode arguments?
		if ( ! empty( $this->config['email_type'] ) ) {
			$hidden_fields .= '<input type="hidden" name="_ema4wp_email_type" value="' . esc_attr( $this->config['email_type'] ) . '" />';
		}
	 ...
	 *
	 * @param EMA4WP_Form_Notice $notice
	 *
	 * @return string
	 */
	protected function get_notice_html( EMA4WP_Form_Notice $notice ) {
		if ( $notice->text === '' ) {
			return '';
		}

		$html = sprintf( '<div class="ema4wp-alert ema4wp-%s" role="alert"><p>%s</p></div>', esc_attr( $notice->type ), $notice->text );
		return $html;
	}
	...
	public function get_response_html( $force_show = false ) {
		 ...
		 * @param EMA4WP_Form $form The form object
		 */
		$html = (string) apply_filters( 'ema4wp_form_response_html', $html, $form );

		// wrap entire response in div, regardless of a form was submitted
		$html = '<div class="ema4wp-response">' . $html . '</div>';
		return $html;
	}
	...
	protected function get_response_position() {
		 ...
		 * @since 2.0
		 */
		$response_position = (string) apply_filters( 'ema4wp_form_response_position', $position, $form );
	...
	protected function get_html_before_fields() {
		 ...
		 * @ignore
		 */
		$html = (string) apply_filters( 'ema4wp_form_before_fields', $html, $form );
	...
	protected function get_html_after_fields() {
		 ...
		 * @ignore
		 */
		$html = (string) apply_filters( 'ema4wp_form_after_fields', $html, $form );
	...
	protected function get_form_element_attributes() {
		 ...
		 * @param EMA4WP_Form $form
		 */
		$form_action_attribute = apply_filters( 'ema4wp_form_action', $form_action_attribute, $form );
		 ...
		 * @param EMA4WP_Form $form
		 */
		$attributes = (array) apply_filters( 'ema4wp_form_element_attributes', $attributes, $form );
	...
	public function generate_html( array $config = null ) {
		...
		$opening_html  = '<!-- ZozoEMA for WordPress v' . EMA4WP_VERSION . ' - https://wordpress.org/plugins/zozoema-for-wp/ -->';
		...
		$closing_html  = '</form><!-- / ZozoEMA for WordPress Plugin -->';
		...
		if ( ! $this->is_submitted
			...
			$fields = '<div class="ema4wp-form-fields">' .
	...
	protected function get_css_classes() {
		...
		$classes[] = 'ema4wp-form';
		$classes[] = 'ema4wp-form-' . $form->ID;

		// Add form classes if this specific form element was submitted
		if ( $this->is_submitted ) {
			$classes[] = 'ema4wp-form-submitted';

			if ( ! $form->has_errors() ) {
				$classes[] = 'ema4wp-form-success';
			} else {
				$classes[] = 'ema4wp-form-error';
			}
		}
		...
		if ( ! empty( $form->settings['css'] ) ) {
			if ( strpos( $form->settings['css'], 'theme-' ) === 0 ) {
				$classes[] = 'ema4wp-form-theme';
			}

			$classes[] = 'ema4wp-form-' . $form->settings['css'];
		}
		 ...
		 * @param EMA4WP_Form $form
		 */
		$classes = apply_filters( 'ema4wp_form_css_classes', $classes, $form );
