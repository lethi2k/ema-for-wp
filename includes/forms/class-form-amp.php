...
class EMA4WP_Form_AMP {
	...
	public function add_hooks() {
		add_filter( 'ema4wp_form_content', array( $this, 'add_response_templates' ), 10, 2 );
		add_filter( 'ema4wp_form_element_attributes', array( $this, 'add_amp_request' ) );
		add_filter( 'ema4wp_load_form_scripts', array( $this, 'suppress_scripts' ) );
	}
	...
	public function add_response_templates( $content, $form ) {
		...
		<div submitting>
			<template type="amp-mustache">
				<?php echo esc_html__( 'Submitting...', 'zozoema-for-wp' ); ?>
			</template>
		</div>
		<div submit-success>
			<template type="amp-mustache">
	...
	public function add_amp_request( $attributes ) {
		if ( function_exists( 'is_amp_endpoint' ) && is_amp_endpoint() ) {
			$attributes['action-xhr'] = get_rest_url( null, 'ema4wp/v1/form' );
		}
