...
/**
 * Class EMA4WP_Form_Output_Manager
 *
 * @ignore
 * @access private
 */
class EMA4WP_Form_Output_Manager {
	...
	public function add_hooks() {
		...
		add_filter( 'ema4wp_form_content', 'do_shortcode' );
		add_action( 'init', array( $this, 'register_shortcode' ) );
	}

	/**
	 * Registers the [ema4wp_form] shortcode
	 */
	...
	public function output_form( $id = 0, $config = array(), $echo = true ) {
		try {
			$form = ema4wp_get_form( $id );
		} catch ( Exception $e ) {
			if ( current_user_can( 'manage_options' ) ) {
				return sprintf( '<strong>ZozoEMA for WordPress error:</strong> %s', $e->getMessage() );
			}
		...
		if ( empty( $config['element_id'] ) ) {
			$config['element_id'] = 'ema4wp-form-' . $this->count;
		}
		...
		try {
			 ...
			 */
			do_action( 'ema4wp_output_form', $form );
