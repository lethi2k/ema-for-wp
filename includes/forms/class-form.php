 ...
 *
 * To get a form instance, use `ema4wp_get_form( $id );` where `$id` is the post ID.
 *
 * @access public
 * @since 3.0
 */
class EMA4WP_Form {
	...
	public static function throw_not_found_exception( $post_id ) {
		$message = sprintf( __( 'There is no form with ID %d, perhaps it was deleted?', 'zozoema-for-wp' ), $post_id );
	...
	public static function get_instance( $post = 0 ) {
		if ( $post instanceof WP_Post ) {
			...
			if ( $post_id === 0 ) {
				$post_id = (int) get_option( 'ema4wp_default_form_id', 0 );
			}
		}
		...
		// check post object
		if ( ! $post instanceof WP_Post || $post->post_type !== 'ema4wp-form' ) {
			self::throw_not_found_exception( $post_id );
		}

		// get all post meta in single call for performance
		$post_meta = (array) get_post_meta( $post_id );
		$form      = new EMA4WP_Form( $post_id, $post, $post_meta );
	...
	*/
	 ...
	 *
	 * @see EMA4WP_Form_Element::get_response_html()
	 *
	 * @return string
	 */
	 ...
	 * @param array $config
	 * @return EMA4WP_Form_element
	 */
	public function get_element( $element_id = 'ema4wp-form', array $config = array() ) {
		return new EMA4WP_Form_Element( $this, $element_id, $config );
	}

	/**
	 * Get HTML string for this form.
	 *
	 * If you want to output a form, use `ema4wp_show_form` instead as it.
	 *
	 * @param string $element_id
	 * @param array $config
	 *
	 * @return string
	 */
	public function get_html( $element_id = 'ema4wp-form', array $config = array() ) {
	...
	protected function load_settings( array $post_meta = array() ) {
		$form             = $this;
		$default_settings = include EMA4WP_PLUGIN_DIR . 'config/default-form-settings.php';

		// start with defaults
		$settings = $default_settings;

		// get custom settings from meta
		if ( ! empty( $post_meta['_ema4wp_settings'] ) ) {
			$meta = $post_meta['_ema4wp_settings'][0];
		 ...
		 * @param EMA4WP_Form $form
		 */
		$settings = (array) apply_filters( 'ema4wp_form_settings', $settings, $form );
	...
	protected function load_messages( array $post_meta = array() ) {
		...
		$default_messages = include EMA4WP_PLUGIN_DIR . 'config/default-form-messages.php';
		 ...
		 * @param EMA4WP_Form $form
		 */
		$messages = (array) apply_filters( 'ema4wp_form_messages', $messages, $form );
	...
	*/
	public function add_notice( $text, $type = 'notice' ) {
		$this->notices[] = new EMA4WP_Form_Notice( $text, $type );
	}
	...
	public function __toString() {
		return ema4wp_show_form( $this->ID, array(), false );
	}
	...
	public function get_redirect_url() {
		 ...
		 * @param EMA4WP_Form $form
		 */
		$url = (string) apply_filters( 'ema4wp_form_redirect_url', $url, $form );
		return $url;
	}
	...
	public function validate() {
		...
		if ( ! isset( $this->raw_data['_ema4wp_timestamp'] ) || $this->raw_data['_ema4wp_timestamp'] > ( time() - 2 ) ) {
			$errors[] = 'spam';
		} elseif ( ! isset( $this->raw_data['_ema4wp_honeypot'] ) || ! empty( $this->raw_data['_ema4wp_honeypot'] ) ) {
			$errors[] = 'spam';
		}
		...
		if ( empty( $errors ) ) {
			...
			foreach ( $this->get_required_fields() as $field ) {
				$value = ema4wp_array_get( $this->data, $field );
		 ...
		 * @param EMA4WP_Form $form
		 */
		$errors = (array) apply_filters( 'ema4wp_form_errors', $errors, $form );

		/**
		 * @ignore
		 * @deprecated 3.0 Use `ema4wp_form_errors` instead
		 */
		$form_validity = apply_filters( 'ema4wp_valid_form_request', true, $this->data );
	...
	public function handle_request( array $data ) {
		...
		$map    = array(
			'_ema4wp_lists'           => 'lists',
			'_ema4wp_action'          => 'action',
			'_ema4wp_form_element_id' => 'element_id',
			'_ema4wp_email_type'      => 'email_type',
		);
	...
	protected function parse_request_data( array $data ) {
		...
		* @param EMA4WP_Form $form The form instance.
		*/
		$ignored_field_names = apply_filters( 'ema4wp_form_ignored_field_names', $ignored_field_names, $form );
	...
	/**
	 * Get ZozoEMA lists this form subscribes to
	 *
	 * @return array
	 */
	public function get_lists() {
		...
		/**
		 * Filters ZozoEMA lists new subscribers should be added to.
		 *
		 * @param array $lists
		 */
		...
		/**
		 * Filters ZozoEMA lists new subscribers coming from this form should be added to.
		 *
		 * @param array $lists
		 * @param EMA4WP_Form $form
		 */
		$lists = (array) apply_filters( 'ema4wp_form_lists', $lists, $form );
	...
	public function get_data() {
		 ...
		 * @param EMA4WP_Form $form
		 */
		$data = apply_filters( 'ema4wp_form_data', $data, $form );
	...
	public function get_required_fields() {
		...
		// workaround for #261 (https://github.com/ibericode/zozoema-for-wordpress/issues/261)
		...
		// fix uppercased subkeys, see https://github.com/ibericode/zozoema-for-wordpress/issues/516
		 ...
		 *
		 * - All fields which are required for the selected ZozoEMA lists
		 * - All fields in the form with a `required` attribute.
		 *
		 * @param array $required_fields
		 * @param EMA4WP_Form $form
		 */
		$required_fields = (array) apply_filters( 'ema4wp_form_required_fields', $required_fields, $form );

		return $required_fields;
	}

	/**
	 * Get "email_type" setting for new ZozoEMA subscribers added by this form.
	 *
	 * @return string
	 */
	...
	*/
	public function get_message( $key ) {
		...
		if ( $key === 'no_lists_selected' && current_user_can( 'manage_options' ) ) {
			$message .= sprintf( ' (<a href="%s">%s</a>)', ema4wp_get_edit_form_url( $this->ID, 'settings' ), 'edit form settings' );
		}
