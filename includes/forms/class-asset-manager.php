 ...
 * @ignore
*/
class EMA4WP_Form_Asset_Manager {
	...
	public function add_hooks() {
		...
		add_action( 'ema4wp_output_form', array( $this, 'before_output_form' ) );
	}
	...
	public function register_scripts() {
		...
		wp_register_script( 'ema4wp-forms-api', EMA4WP_PLUGIN_URL . 'assets/js/forms' . $filename_suffix . '.js', array(), EMA4WP_VERSION, true );
	}
	...
	public function get_stylesheet_url( $stylesheet ) {
		...
		return EMA4WP_PLUGIN_URL . 'assets/css/form-' . $stylesheet . '.min.css';
	}
	...
	public function get_active_stylesheets() {
		$stylesheets = (array) get_option( 'ema4wp_form_stylesheets', array() );
		 ...
		 * Each value is prefixed with `ema4wp-form-` to get the handle.
		 *
		 ...
		 */
		$stylesheets = (array) apply_filters( 'ema4wp_form_stylesheets', $stylesheets );
		return $stylesheets;
	}
	...
	public function load_stylesheets() {
		...
		foreach ( $stylesheets as $stylesheet ) {
			...
			$handle = 'ema4wp-form-' . $stylesheet;
			wp_enqueue_style( $handle, $this->get_stylesheet_url( $stylesheet ), array(), EMA4WP_VERSION );
		 ...
		 * @ignore
		 */
		do_action( 'ema4wp_load_form_stylesheets', $stylesheets );
	...
	public function get_submitted_form_data() {
		$submitted_form = ema4wp_get_submitted_form();
		if ( ! $submitted_form instanceof EMA4WP_Form ) {
			return null;
		}
		 ...
		 * @since 3.0
		 */
		$data['auto_scroll'] = apply_filters( 'ema4wp_form_auto_scroll', $data['auto_scroll'] );
	...
	public function before_output_form() {
		$load_scripts = apply_filters( 'ema4wp_load_form_scripts', true );
		if ( ! $load_scripts ) {
			return;
		}
	...
	/**
	 * Prints dummy JavaScript which allows people to call `ema4wp.forms.on()` before the JS is loaded.
	 */
	...
	*/
	public function load_scripts() {
		$load_scripts = apply_filters( 'ema4wp_load_form_scripts', $this->load_scripts );
		if ( ! $load_scripts ) {
			return;
		}

		// load general client-side form API
		wp_enqueue_script( 'ema4wp-forms-api' );
		...
		if ( $submitted_form_data !== null ) {
			$filename_suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
			wp_enqueue_script( 'ema4wp-forms-submitted', EMA4WP_PLUGIN_URL . 'assets/js/forms-submitted' . $filename_suffix . '.js', array( 'ema4wp-forms-api' ), EMA4WP_VERSION, true );
			wp_localize_script( 'ema4wp-forms-submitted', 'ema4wp_submitted_form', $submitted_form_data );
		}
		...
		/** @ignore */
		do_action( 'ema4wp_load_form_scripts' );
	}
}
