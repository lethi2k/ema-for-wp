...
/**
 * Adds EMA4WP_Widget widget.
 *
 * @ignore
 */
class EMA4WP_Form_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$this->default_instance_settings['title'] = __( 'Newsletter', 'zozoema-for-wp' );

		parent::__construct(
			'ema4wp_form_widget', // Base ID
			__( 'ZozoEMA Sign-Up Form', 'zozoema-for-wp' ), // Name
			array(
				'description' => __( 'Displays your ZozoEMA for WordPress sign-up form', 'zozoema-for-wp' ),
			)
		);
	}
	...
	public function widget( $args, $instance_settings ) {
		...
		ema4wp_show_form( $instance_settings['form_id'] );
	...
	public function form( $settings ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'zozoema-for-wp' ); ?></label>
		 ...
		 * @param EMA4WP_Form_Widget $this
		 * @ignore
		 */
		do_action( 'ema4wp_form_widget_form', $settings, $this );
		?>

		<p class="help">
			<?php printf( __( 'You can edit your sign-up form in the <a href="%s">ZozoEMA for WordPress form settings</a>.', 'zozoema-for-wp' ), admin_url( 'admin.php?page=zozoema-for-wp-forms' ) ); ?>
		</p>
		<?php
	}
	...
	public function update( $new_settings, $old_settings ) {
		 ...
		 * @param EMA4WP_Form_Widget $widget
		 * @ignore
		 */
		$new_settings = apply_filters( 'ema4wp_form_widget_update_settings', $new_settings, $old_settings, $this );
