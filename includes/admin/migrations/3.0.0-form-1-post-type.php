...
// get options
$form_options = get_option( 'ema4wp_lite_form', array() );
...
if ( ! empty( $has_forms ) ) {
	...
	delete_option( 'ema4wp_lite_form' );
	return;
}
...
$id = wp_insert_post(
	array(
		'post_type'    => 'ema4wp-form',
		'post_status'  => 'publish',
		'post_title'   => __( 'Default sign-up form', 'zozoema-for-wp' ),
		'post_content' => ( empty( $form_options['markup'] ) ) ? '' : $form_options['markup'],
	)
);

// set default_form_id
update_option( 'ema4wp_default_form_id', $id );
...
update_post_meta( $id, '_ema4wp_settings', $settings );
