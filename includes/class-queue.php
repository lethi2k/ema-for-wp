...
class EMA4WP_Queue {
	...
	/**
	 * EMA4WP_Ecommerce_Queue constructor.
	 *
	...
	protected function load() {
		...
		if ( ! is_array( $jobs ) ) {
			...
			foreach ( $jobs as $i => $obj ) {
				...
				// make sure each job is instance of EMA4WP_Queue_Job
				if ( $obj instanceof EMA4WP_Queue_Job ) {
					$job = $obj;
				} else {
					$job     = new EMA4WP_Queue_Job( $obj->data );
					$job->id = $obj->id;
				}
	 ...
	 *
	 * @return EMA4WP_Queue_Job[] Array of jobs
	 */
	public function all() {
		$this->load();
		return $this->jobs;
	...
	/**
	 * @param EMA4WP_Queue_Job $job
	 */
	public function delete( EMA4WP_Queue_Job $job ) {
		$this->load();
	 ...
	 * @param EMA4WP_Queue_Job $job
	 */
	public function reschedule( EMA4WP_Queue_Job $job ) {
		$this->load();
